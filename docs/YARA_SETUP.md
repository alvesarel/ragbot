...
└─────────┬─────────────────────────────────┬─────────────────┘
          ...
          ▼                                 ▼
┌─────────────────────────┐      ┌─────────────────┐
│   PRE-FETCHED DATA      │      │  KNOWLEDGE BASE │
│ (Patients + Check-ins)  │      │     (RAG)       │
│   from Google Sheets    │      │    Qdrant       │
└─────────────────────────┘      └─────────────────┘
```

Yara uses a consolidated single-workflow architecture:
- Patient data and check-in logs are pre-fetched in parallel on each message
- Pre-calculated analytics (summaries, renewals, follow-ups) are injected into the AI context
- Only the RAG tool is used for knowledge base queries
...
3. **Knowledge Base** populated in Qdrant (`clinic_knowledge_base` collection)
...
3. Select `yara-executive-assistant.json`
4. Click **Save**

No sub-workflows needed - Yara is now a single consolidated workflow.
...
1. Open `Yara - Executive Assistant AI Agent`
...
4. Check Google Sheets credentials are valid
5. Verify `PATIENT_SHEET_ID` points to correct spreadsheet

### RAG Not Working
1. Verify Qdrant connection is working
2. Ensure `clinic_knowledge_base` collection exists and is populated
3. Run the knowledge base setup workflow or embedding script if needed
